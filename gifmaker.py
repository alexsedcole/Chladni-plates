...
class Plate:
            ...
            for x in range(1, self.nx-1):
                for y in range(1, self.ny-1):
                    if x != int(self.nx/2) or y != int(self.ny/2):
                        ...
                        U[t,x,y] = 2*U[t-1,x,y] - U[t-2,x,y] + self.k**2*uxx + self.k**2*uyy
    ...
    def visualise(self):
        ...
        #'''
        fig = plt.figure(figsize=(10, 7))  # Set the figure size to fullscreen
        ...
        fig.colorbar(matplotlib.cm.ScalarMappable(norm=norm, cmap=colour), ax=ax3, orientation='vertical', label='Displacement (m)', extend='both')
